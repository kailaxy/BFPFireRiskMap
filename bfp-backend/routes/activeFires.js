...
const { getActiveFiresFeatureCollection } = require('../services/firestoreRouteHelpers');

function buildActiveFiresReadError(err, provider) {
  return {
    endpoint: '/api/active_fires',
    provider,
...
router.get('/', async (req, res) => {
  ...
  try {
    if (provider === 'firestore') {
      const featureCollection = await getActiveFiresFeatureCollection();
    ...
    console.error('Error fetching active_fires', buildActiveFiresReadError(err, provider));
    return res.status(500).json({
