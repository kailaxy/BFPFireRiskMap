...
const { getBarangaysFeatureCollection } = require('../services/firestoreRouteHelpers');

function buildBarangaysReadError(err, provider) {
  return {
...
router.get('/', async (req, res) => {
  ...
  try {
    if (provider === 'firestore') {
      const featureCollection = await getBarangaysFeatureCollection();
    ...
    console.error('Error fetching barangays', buildBarangaysReadError(err, provider));
