...
const { getFireStationsFeatureCollection } = require('../services/firestoreRouteHelpers');

function buildFireStationsReadError(err, provider) {
  return {
...
function extractStationPoint(feature) {
  ...
  const point = candidates.find((candidate) => (
    candidate && isValidPoint(candidate.lat, candidate.lng)
  ));

  return point || null;
}

function withStationCoordinates(featureCollection) {
  const features = Array.isArray(featureCollection?.features) ? featureCollection.features : [];

  return {
    type: 'FeatureCollection',
    features: features.map((feature) => {
      const point = extractStationPoint(feature);
      ...
      return {
        ...
        properties: {
          ...(feature.properties || {}),
          latitude: toFiniteNumber(feature?.properties?.latitude) ?? lat,
          longitude: toFiniteNumber(feature?.properties?.longitude) ?? lng,
        },
      };
    }),
  };
...
router.get('/', async (req, res) => {
  const provider = isFirestoreProvider() ? 'firestore' : 'postgres';

  try {
    if (provider === 'firestore') {
      const featureCollection = await getFireStationsFeatureCollection();
      return res.json(withStationCoordinates(featureCollection));
    }
    ...
    console.error('Error fetching fire stations', buildFireStationsReadError(err, provider));
