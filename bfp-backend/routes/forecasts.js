  ...
  try {
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({ 
        ...
        hint: 'Generate forecasts first to populate forecast data'
      });
    }
    ...
    if (error.code === 'UNDEFINED_TABLE' || error.message?.includes('does not exist')) {
      return res.status(503).json({ 
        error: 'Forecasts table not yet initialized',
        hint: 'Please run forecast generation to populate the database',
        code: 'FORECASTS_TABLE_NOT_FOUND'
      });
    }
