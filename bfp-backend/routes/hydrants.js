...
const { getHydrantsFeatureCollection } = require('../services/firestoreRouteHelpers');

function buildHydrantsReadError(err, provider) {
  return {
...
router.get('/', async (req, res) => {
  ...
  try {
    if (provider === 'firestore') {
      const featureCollection = await getHydrantsFeatureCollection();
    ...
    console.error('Error fetching hydrants', buildHydrantsReadError(err, provider));
    return res.status(500).json({
      error: 'Failed to fetch hydrants',
      provider,
    });
  }
});
