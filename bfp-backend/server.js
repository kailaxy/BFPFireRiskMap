...
// Debug endpoint: Check data provider
app.get('/api/debug/data-provider', (req, res) => {
  res.json({
    DATA_PROVIDER: process.env.DATA_PROVIDER || 'not-set',
    isFirestoreProvider: isFirestoreProvider(),
...
app.listen(PORT, HOST, () => {
  ...
  // Initialize Firestore if provider is enabled
  if (isFirestoreProvider()) {
    try {
      require('./config/firestore').initializeFirestore();
      console.log('🔥 Firestore client initialized successfully');
    } catch (error) {
      console.error('❌ Failed to initialize Firestore:', error.message);
      console.error('⚠️  Falling back to PostgreSQL provider');
    }
  }
...
app.get('/api/admin/generate-monthly-report-simple-fix', async (req, res) => {
  try {
    ...
    if (occupancy) {
      // Column not available in production dataset; keep request compatibility by ignoring this filter.
    }
