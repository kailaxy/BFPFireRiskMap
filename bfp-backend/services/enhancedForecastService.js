  ...
  async storeForecastsInDatabase(forecasts, metadata) {
    ...
    try {
      ...
      for (const forecast of forecasts) {
        ...
        const insertQuery = `
          -- Future rows are deleted first, so a plain insert is sufficient here.
        ...
        const responseSignature = (() => {
          try {
            return JSON.stringify({
              keys: Object.keys(batch || {}).slice(0, 10),
              detail: typeof batch?.detail === 'string' ? batch.detail.slice(0, 200) : null,
              all_forecasts_type: Array.isArray(batch?.all_forecasts) ? 'array' : typeof batch?.all_forecasts,
              all_forecasts_count: Array.isArray(batch?.all_forecasts) ? batch.all_forecasts.length : null,
              total_predictions: batch?.total_predictions ?? null,
            });
          } catch (_) {
            return '[unserializable-response]';
          }
        })();
        console.log(`   🧾 Microservice response signature: ${responseSignature}`);

        if (typeof batch?.detail === 'string' && batch.detail.trim()) {
          throw new Error(`Forecast microservice error: ${batch.detail.slice(0, 300)} | signature=${responseSignature}`);
        }
        if (!batch || !Array.isArray(batch.all_forecasts)) {
          throw new Error(`Forecast microservice returned invalid payload: expected all_forecasts array | signature=${responseSignature}`);
        }
